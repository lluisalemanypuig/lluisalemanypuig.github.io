...
function setCounterText(n) {
	...
	itemCount.appendChild(
		document.createTextNode("Number of items returned: " + n.toString())
	);
}
...
var authorTagClicked = function (event) {
	...
	setSelection(
		document.getElementById(__pubs_dd_authors_id),
...
var typeTagClicked = function (event) {
	...
	setSelection(
		document.getElementById(__pubs_dd_tags_id),
		event.target.textContent
	);
	populatePublicationList();
};
...
function format_preprint_generic(par, work) {
	...
	// where published
	{
		...
		preprint_italics.textContent = full_name_plus_short(
			CITE.repository,
			__publishedin_relate[CITE.repository]
		);
...
function format_journal_generic(par, work) {
	...
	par.appendChild(document.createTextNode(" "));
	{
		...
		journal_italics.textContent = full_name_plus_short(
			CITE.journal,
			__publishedin_relate[CITE.journal]
		);
...
function format_conference_proceedings(par, work) {
	...
	if (CITE.proceedings_url != null) {
		...
		var url_ref = document.createElement("a");
		url_ref.textContent = full_name_plus_short(
			CITE.proceedings,
			__publishedin_relate[CITE.proceedings]
		);
		...
		var proceedings_italics = document.createElement("i");
		proceedings_italics.textContent = full_name_plus_short(
			CITE.proceedings,
			__publishedin_relate[CITE.proceedings]
		);
...
function populatePublicationList() {
	...
	console.log(
		"    Filtering works... (" + Object.keys(works).length + " found)"
	);
	...
	for (var i = 0; i < Object.keys(works).length; ++i) {
		...
		if (to_be_included) {
			console.log(
				"        Item: " + key + " is to be included in the list"
			);
			worksList.push(key);
		} else {
			console.log(
				"        Item: " + key + " is NOT to be included in the list"
			);
			console.log("            Reason:");
