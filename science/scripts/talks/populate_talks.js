...
var authorTagClicked = function (event) {
	...
	setSelection(
		document.getElementById(__talks_dd_authors_id),
...
var typeTagClicked = function (event) {
	...
	setSelection(
		document.getElementById(__talks_dd_tags_id),
		event.target.textContent
	);
	populateTalksList();
};
...
function makeFormattedTalk(talkid, TALK) {
	...
	if (TALK.session != null) {
		par.appendChild(
			document.createTextNode(" Session: " + TALK.session + ".")
		);
	}

	// add date
	par.appendChild(
		document.createTextNode(" " + TALK.year + " (" + TALK.date + ")")
	);
...
function populateTalksList() {
	...
	console.log(
		"    Contents of 'seminar conference' drop down: " + use_semconf
	);
