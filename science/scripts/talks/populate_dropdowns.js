...
function populateDropDowns() {
	...
	all_semconf.sort(function (a, b) {
		return a.localeCompare(b);
	});
	all_insts.sort(function (a, b) {
		return a.localeCompare(b);
	});
	...
	console.log(
		"    Add " + all_locations.length + " cities: " + all_locations
	);
	...
	all_years.forEach(function (item) {
		addToDropDown(ddYears, item);
	});
	all_tags.forEach(function (item) {
		addToDropDown(ddTags, item);
	});
	all_semconf.forEach(function (item) {
		addToDropDown(ddSeminarConf, item);
	});
	all_insts.forEach(function (item) {
		addToDropDown(ddInstitutions, item);
	});
	all_locations.forEach(function (item) {
		addToDropDown(ddCities, item);
	});
	...
	console.log(
		"    Values in seminars/conferences drop down: " +
			ddSeminarConf.childNodes.length
	);
	console.log(
		"    Values in institutions drop down: " +
			ddInstitutions.childNodes.length
	);
	console.log(
		"    Values in cities drop down: " + ddCities.childNodes.length
	);
	console.log(
